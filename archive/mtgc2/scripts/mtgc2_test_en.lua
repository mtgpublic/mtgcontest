...
S.events:getListeners(C.TowerBuild):add(C.Listener(function(event)
    ...
    if type == C.TowerType.CANNON or type == C.TowerType.FREEZING or type == C.TowerType.BLAST or type == C.TowerType.MULTISHOT or type == C.TowerType.GAUSS or type == C.TowerType.LASER or type == C.TowerType.CRUSHER then
        valid = 0
        C.Notifications:i():addFailure("Illegal tower build")
        faliure_reason = "Illegal tower build"
...
S.events:getListeners(C.AbilityApply):add(C.Listener(function(event)
    ...
    if type ~= C.AbilityType.LOIC and type ~= C.AbilityType.BULLET_WALL then
        valid = 0
        C.Notifications:i():addFailure("Illegal ability usage")
        faliure_reason = "Illegal ability usage"
        S.gameState:setGameSpeed(0)
    end
end))
...
S.events:getListeners(C.ModifierBuild):add(C.Listener(function(event)
    ...
    if modifier_count > 4 then
        valid = 0
        C.Notifications:i():addFailure("Modifier build category limit exceeded")
        C.Notifications:i():addFailure("Current number of modifier build categories: " .. tostring(modifier_count))
        faliure_reason = "Modifier build category limit exceeded"
        S.gameState:setGameSpeed(0)
    end
end))
S.events:getListeners(C.GameOver):add(C.Listener(function(event)
    if valid == 0 then
        C.Notifications:i():addFailure("This game run is invalid")
        C.Notifications:i():addFailure("Most recent invalid action details:" .. faliure_reason)
    else
        C.Notifications:i():addSuccess("This game run has been validated successfully")
    end
end))